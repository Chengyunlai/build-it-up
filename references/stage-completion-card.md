...
### Example 双层结构
- user_code 主线：<使用者代码路径、公开 API 调用、输入和输出；这是本阶段首先阅读和运行的部分>
- core 核心代码：<本次核心实现路径、稳定符号、关键状态和 C1 映射；这是解释 user_code 如何工作的部分>
- 清爽度结论：<user_code 是否只表达使用者意图；如果不够清爽，记录公开 API / 组装边界问题和处理结果>

### 关键路径图
<一张展示真实输入、实际处理和可观察输出的图>

### 关键路径断点
断点顺序优先从 `user_code/` 进入公开 API，再进入 `core/` 的核心行为、状态变化和输出；不要把内部断点写成使用者必须理解的前置步骤。
1. 断点：<可以在哪里暂停或检查>
