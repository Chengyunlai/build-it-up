...
它还维护一条可回放的实现闭环：issue / 设计记录说明为什么做，统一目录中的 example 先用 `user_code/` 展示使用者如何清爽地调用，再用 `core/` 解释本次核心代码如何工作；断点指南说明如何观察，测试提供证据，commit 固化变更，最终实现记录把这些对象重新串起来。项目 README、context 和 AGENTS / 等价 agents 文件会说明这些入口，让其他 IDE 和 AI 不必依赖原始对话才能接手。
...
图表还会根据本次任务自动取名。稳定的文件 / 页面名用于 issue 互链；画布底部会单独放置“图名：具体任务结果｜图型：图型｜阶段：阶段标识”，让读者在导出图片、切换页面或脱离聊天后仍知道这张图在解释什么。名称不会使用空泛的“流程图”“系统架构”作为唯一标题。选图矩阵、官方示例和命名细则见 [`references/diagram-selection.md`](references/diagram-selection.md)。

这里的“图”按问题选择：整体架构用分层图表达归属和边界，对象定义用 UML 类图，调用顺序用 UML 时序图。输入、处理、结果是行为图的要求，不强制塞入架构图和类图。标准符号与验收见 [图型规范](references/diagram-selection.md)，示范见 [中性示例](references/diagram-examples.md)。

例如，行为图不画“验证应用 → 尽快获得证据”，而画：
...
已确认范围内的图型、布局与文字修正直接执行；解释或评估问题直接回答，不反复启动确认流程。新目标、范围变化和新的外部写入仍需相应授权。
...
每个阶段 example 都必须有两个部分：`user_code/` 是使用者视角的主线，只通过公开 API 完成一条真实调用；`core/` 是本次核心代码和实现映射。先运行 `user_code/`，如果这段代码不清爽，就先回到公开 API 和组装边界修正设计，不用长篇注释掩盖问题。完整契约见 [`references/example-contract.md`](references/example-contract.md)。
...
flowchart TD
    ...
    I --> G
    H -- 是 --> J[创建或更新 issue]
    J --> K{是否检测到可选图表载体且用户确认导入}
    K -- 否 --> L[交付 issue 和行动指南]
    K -- 是 --> M[导入正确图型、加入底部任务名并验证]
    M --> L
    L --> N{是否另外要求实现}
    ...
    Q --> R{是否生成阶段完成卡}
    R -- 否 --> S[普通完成摘要并结束]
    R -- 是 --> T[输出 user_code、core、断点、图和验证证据]
...
当用户只说“画图”时，skill 仍会先根据问题选择图型并生成 Mermaid；如果实际检测到网页版 `app.diagrams.net` 或其他可用 draw.io 载体，会主动提示一次是否导入。用户确认后才执行导入，并在画布底部加入任务名称和图型名称，然后回读验证；用户拒绝后不再重复询问。
...
└── references/
    ...
    ├── diagram-selection.md          # 图型选择、任务命名和官方质量参考
    ...
    └── example-contract.md           # core 与 user_code 的 example 契约
```
