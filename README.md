...
它的核心原则是：先把想法讲清楚，再建立 issue 和图表，得到用户明确确认后，才进行外部写入；如果用户另外要求，才进入写代码阶段。
...
```mermaid
flowchart TD
    A[用户提出需求] --> B[明确问题、目标和范围]
    B --> C[用通俗语言解释实现方案]
    C --> D[生成 Mermaid 图]
    D --> E[起草 issue 和验收标准]
    E --> F{用户是否明确确认}
    F -- 否 --> G[根据反馈修订方案、图和 issue]
    G --> F
    F -- 是 --> H[创建或更新 issue]
    H --> I{是否要求绑定 draw.io}
    I -- 否 --> J[交付 issue 和行动指南]
    I -- 是 --> K[连接网页版或桌面版 draw.io]
    K --> L[验证图表并建立双向链接]
    L --> J
    J --> M{是否另外要求实现}
    M -- 是 --> N[按已确认的 issue 开始编码]
    M -- 否 --> O[流程结束]
```
