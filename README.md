...
它的核心原则是：先帮助人理解当前唯一需要做的决定，再逐步建立共识、issue 和图表；得到用户明确确认后，才进行外部写入；如果用户另外要求，才进入写代码阶段。
...
```mermaid
flowchart TD
    A[用户提出需求] --> B[展示第一张决策卡片]
    B --> C{用户是否确认当前决策}
    C -- 否 / 有疑问 --> D[只解释当前疑问并修订卡片]
    D --> B
    C -- 是 --> E[进入下一个唯一决策]
    E --> F{关键决策是否已稳定}
    F -- 否 --> B
...
skill 不会把完整 issue 当作第一轮聊天的入口。第一轮优先展示一张决策卡片，只回答：
