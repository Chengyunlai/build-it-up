...
- **新需求共识**：目标或范围尚不清楚时，使用决策卡片和有限确认流程。
- **解释 / 评估**：用户只问概念、评价或原因时，直接回答，不强制创建 issue、绘图或重新确认。
- **已授权交付 / 修订**：用户已明确要求画图、修改 skill 或修正已确认产物时，直接在授权范围执行。图型、换行、布局修正无需再次确认；目标、范围、发布目的地或重要行为改变时才重新确认。不要把本 skill 的讨论流程递归套用到对本 skill 的维护。

以下第一轮决策卡与完整共识流程适用于“新需求共识”，不覆盖上面的直接交付模式。无论哪种模式，授权都不自动扩大到发布、推送或修改其他系统。
...
图必须展示可核实的事实，但不同图型有不同的证据：

| 图型 | 必须看清什么 | 不强制包含 |
| --- | --- | --- |
| 分层架构图 | 职责层、模块归属、系统边界、关键依赖或扩展接口 | 单次请求、输入值、执行结果 |
| UML 类图 | 类 / 接口、必要属性和方法、标准关系及适用的多重性 | 调用先后与状态轨迹 |
| UML 时序图 | 参与者、生命线、调用与返回、顺序及关键条件分支 | 所有模块与字段 |
| 流程图 / 数据流图 | 具体输入、处理或流转、可观察结果及适用失败边界 | 全部静态结构 |
| 状态图 | 状态、触发事件、守卫条件、合法终态 | 固定输入输出两端 |

先判断读者问的是“系统由什么组成”还是“一次请求怎么执行”，再选图。不能给架构图或类图强塞输入和结果，也不能用自定义方框连线冒充 UML。标准符号、模板和分图验收见 [references/diagram-selection.md](references/diagram-selection.md)。
...
- “先做什么、之后发生什么” → 流程图或时间线。
- “整体架构 / 分层与模块归属” → 分层架构图；明确要求 C4 时使用相应 C4 视图。
- “对象如何定义 / 类与接口关系” → UML 类图。
- “谁先调用谁 / 如何注册和使用” → UML 时序图。
- “系统有哪些状态、如何变化” → 状态图。
...
图必须服务于当前唯一决策，而不是装饰。决策草图通常控制在 3–8 个主要元素；架构总览按职责分区控制复杂度，不为凑节点数省略必要边界；如果一张图无法同时表达“具体场景如何运行”和“本阶段验证边界”，拆成两张互补图，例如“具体执行图 + 验证覆盖图”。每张图都要有一个短标题，并在图下只解释读者必须知道的箭头、边界或差异。
...
当用户改变目标、范围或重要行为时，同步检查摘要、假设、图表、验收标准与实现顺序，再确认变化部分。已授权的图型、布局和文字修正直接执行，不重走完整共识流程；新外部写入仍需相应授权。
